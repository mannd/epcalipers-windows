...
- *Clear calibration each page* Clears calibration each time you change PDF pages if true.  If false, calibration is maintained between pages.
- *PDF Resolution* Set to high for maximum size and resolution of PDF pages.  Set to low for a small page size.  Use low resolution if there are program crashes due to running out of memory.
